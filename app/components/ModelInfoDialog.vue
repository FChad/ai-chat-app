<template>
    <USheet :open="isOpen" side="right" @update:open="(val: boolean) => { if (!val) closeDialog() }" panel-class="w-full max-w-2xl flex flex-col p-0 gap-0">
        <!-- Fixed Header -->
        ...
        <div class="flex-1 overflow-y-auto p-6 scrollbar-thin">
            <div v-if="model" class="space-y-6">
                ...
                <div>
                    <div class="flex items-start justify-between gap-3 mb-2">
                        ...
                        <div class="flex items-center gap-1.5 shrink-0">
                            <UBadge>{{ model.details.family }}</UBadge>
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                <UBadge v-for="m in model.details.architecture.input_modalities" :key="m" variant="secondary">{{ m }}</UBadge>
                            </div>
                        </div>
                        <div>
                            <p class="text-xs text-muted-foreground mb-1.5">Output Modalities</p>
                            <div class="flex flex-wrap gap-1.5">
                                <UBadge v-for="m in model.details.architecture.output_modalities" :key="m" variant="outline">{{ m }}</UBadge>
                            </div>
                        </div>
                ...
                <div v-if="model.details.supported_parameters && model.details.supported_parameters.length > 0">
                    ...
                    <div class="flex flex-wrap gap-1.5">
                        <UBadge v-for="param in model.details.supported_parameters" :key="param"
                            variant="secondary" :title="getParameterDescription(param)">{{ param }}</UBadge>
                    </div>
                </div>
        ...
        <div class="p-4 border-t border-border shrink-0">
            <UButton variant="outline" @click="closeDialog" class="w-full">Close</UButton>
        </div>
    </USheet>
</template>
